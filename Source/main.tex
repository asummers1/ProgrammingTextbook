...
With this in mind, create a diagram that swaps two numbers. A good way to draw this out might be to represent two variables as boxes with arbitrary numbers inside them, and visualize how you might swap the values. Note that setting a variable's value equal to another will overwrite the original value, so you might need more than two variables in your solution.
\exercise
...
Feel free to hard-code in the first two values into variables: 1 and 1. You can display the first value (initially the first 1) before a loop, and display and calculate subsequent values in a loop.
...
\end{adjustbox}
\exercise
What data type did we use at the beginning of this chapter? What error appears in Visual Studio when you try to use a different data type for those variables?
\exercise
...
You can also perform arithmetic computations with variables. Here's how you can add two numbers, where x and y are initialized to 5 and 7 respectively:
...
\exercise
Try using Console.WriteLine with no argument provided. Explain why you think the result is what it is.
...
\index{string interpolation} String interpolation allows us to combine both variables and string literals into the same WriteLine method. We start by placing a \$ sign at the beginning of the WriteLine argument and continuing with double quotes:
Console.WriteLine(\$"");
...
\exercise
You may have noticed that the quotient from the previous exercise displays the value as a whole number. Why is that? How could you change it to display it with decimals?

\exercise
Create a program that accepts a number, calculates the whole number and decimal portions of the number, and outputs both on separate lines. Solving the decimal portion will require some thought about potential usage of the previously-described operators.
...
string fullName = CreateName("Frank", "Reynolds");

Console.WriteLine($"Hello, {CreateName("Jerome", "Horwitz")}!");
...
\exercise
Write a method that calculates the simple interest on a payment. The formula is
\[
P(1+rt)
\]
where $P$ is the initial principle balance, $r$ is the annual interest rate, and $t$ is the time, in years.
\exercise
...
\exercise
Suppose that you had an array of integers that, supposedly, consisted of the values from 1 to 100. But wait -- one of the values is missing. Write a program that, given such an array, calculates the missing value. 
\exercise
Write a program that, given a string representing a binary number, the value is increased by one.

Solving this will involve noticing a pattern in the increasing of binary digits. See this decimal to binary conversion chart:
...
\begin{verbatim}
    ...
    8: 1000
\end{verbatim}
You can assume that all possible input values are comprised of four digits.
...
We can write text to the file with StreamWriter's WriteLine method. Since this method isn't static, we access it through a StreamWriter object rather than its class.
...
\minitoc
\section{Introduction}
Suppose we had a list of strings, that we, say, obtain from a file:
\begin{CSharp}
...
\par\noindent\rule{\textwidth}{0.4pt}
Another element that you can add is a Label. Unlike Buttons and TextBoxes, what labels do might not be intuitive at first glance. They actually allow you to add text into your program: for example, a descriptor describing another element. We can add them similarly to how we did for Buttons and TextBoxes, and like Buttons, we can set their text with the Content property.
